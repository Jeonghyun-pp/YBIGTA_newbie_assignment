...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h3 = z2 @ W3 + b3
        z3 = tanh(h3 + h1)  # residual connection

        h4 = z3 @ W4 + b4
        y_hat = sigmoid(h4)
        y_hat = y_hat.reshape(X.shape[0],)  # reshape to (N,)
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        # Layer 3: z3 = tanh(h3 + h1), residual connection
        dh3 = dz3 * (1 - z3**2)  # tanh derivative
        db3 = np.sum(dh3, axis=0)
        dW3 = z2.T @ dh3 + 2 * L2_norm * W3
        dz2 = dh3 @ W3.T
        
        # Layer 2: z2 = LeakyReLU(h2)
        dh2 = dz2 * ((h2 > 0) + 0.01 * (h2 <= 0))  # LeakyReLU derivative
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T
        
        # Layer 1: z1 = ReLU(h1), and residual connection from z3
        dh1_residual = dz3 * (1 - z3**2)  # from residual connection
        dh1 = dz1 * (h1 > 0) + dh1_residual  # ReLU derivative + residual
