...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    if EMBEDDINGS_PATH.exists() and IDS_PATH.exists():
        ...
        if cached_ids == ids:
            # Same IDs, return cached embeddings
            return np.load(EMBEDDINGS_PATH)
    
    # Get API keys and create clients
    ...
    # Process in batches with parallel clients
    all_embeddings = []
    ...
    def process_batch(batch_idx, batch_texts):
        ...
        time.sleep(MIN_INTERVAL)  # Rate limiting
        return embeddings
    ...
    with ThreadPoolExecutor(max_workers=num_clients) as executor:
        futures = {executor.submit(process_batch, batch_idx, batch_texts): batch_idx 
        ...
        for future in tqdm(as_completed(futures), total=len(batches), desc="Embedding"):
            ...
            if progress_callback:
                progress_callback(completed * BATCH_SIZE, len(texts))
